...
router.get('/topic',verifyToken,requireRole('ผู้รับการประเมินผล'),async(req,res)=>{
    try {
        ...
        console.error('Errror get topics',error)
...
router.post('/save',verifyToken,requireRole('ผู้รับการประเมินผล'),async(req,res)=>{
    try {
        ...
        for(const item of scores){
            const filrname = fileMap[item.file_key]
            await db.query(`indert into tb_evadetail (id_eva,id_indicate,status_eva,score_member,detail_eva,file_eva) values(?,?,?,?,?,?)`,[id_eva,item.id_indicate,1,item.score,item.detail_eva,filename])
        }
        const [[sumRow]] = await
    } catch (error) {
